...
//#endif
    ...
    if(vic != HDMI_Unkown) {
        printk("hdmitx: ALREADY init VIC = %d\n", vic);
...
static unsigned char hdmitx_m3_getediddata(hdmitx_dev_t* hdmitx_device)
{
    if(hdmi_rd_reg(TX_HDCP_ST_EDID_STATUS) & (1<<4)){
        hdmi_wr_reg(TX_HDCP_MEM_CONFIG, 0);
        if((hdmitx_device->cur_edid_block+2)<=EDID_MAX_BLOCK){
            int ii, jj;
            for(jj=0;jj<2;jj++){
#ifdef LOG_EDID
        ...
        hdmi_wr_reg(TX_HDCP_MEM_CONFIG, 0x2);
        return 1;
    }
    else{
        return 0;
...
static int hdmitx_cntl_ddc(hdmitx_dev_t* hdmitx_device, unsigned cmd, unsigned argv)
{
    ...
    if(!(cmd & CMD_DDC_OFFSET))
        printk("hdmitx: ddc: " "w: invalid cmd 0x%x\n", cmd);
    else
        printk("hdmitx: ddc: " "cmd 0x%x\n", cmd);
    ...
    case DDC_HDCP_GET_AUTH:
        ...
        break;
    default:
        printk("hdmitx: ddc: " "unknown cmd: 0x%x\n", cmd);
    }
    return 1;
}
...
static int hdmitx_cntl_config(hdmitx_dev_t* hdmitx_device, unsigned cmd, unsigned argv)
{
    if(!(cmd & CMD_CONF_OFFSET))
        printk("hdmitx: config: " "hdmitx: w: invalid cmd 0x%x\n", cmd);
    else
        printk("hdmitx: config: " "hdmitx: conf cmd 0x%x\n", cmd);
    ...
    case CONF_CLR_AUDINFO_PACKET:
        ...
        break;
    default:
        printk("hdmitx: config: ""hdmitx: unknown cmd: 0x%x\n", cmd);
    }
    return 1;
}
...
static int hdmitx_cntl_misc(hdmitx_dev_t* hdmitx_device, unsigned cmd, unsigned argv)
{
    if(!(cmd & CMD_MISC_OFFSET))
        printk("hdmitx: misc: " "hdmitx: w: invalid cmd 0x%x\n", cmd);
    else
        printk("hdmitx: misc: " "hdmitx: misc cmd 0x%x\n", cmd);
    ...
    case MISC_TMDS_PHY_OP:
        ...
        break;
    default:
        printk("hdmitx: misc: " "hdmitx: unknown cmd: 0x%x\n", cmd);
    }
    return 1;
}
...
static int hdmitx_get_state(hdmitx_dev_t* hdmitx_device, unsigned cmd, unsigned argv)
{
    if(!(cmd & CMD_STAT_OFFSET))
        printk("hdmitx: stat: " "hdmitx: w: invalid cmd 0x%x\n", cmd);
    else
        printk("hdmitx: stat: " "hdmitx: misc cmd 0x%x\n", cmd);
