...
static int uboot_display_already(tvmode_t mode)
{
    ...
    // check I mode
    if((mode == TVMODE_480I) || (mode == TVMODE_576I)) {
        while(s->reg != MREG_END_MARKER) {
            if(s->reg == ENCI_SYNC_HSO_BEGIN) {
                pxcnt_tab = s->val;
            }
            if(s->reg == ENCI_SYNC_HSO_END) {
                lncnt_tab = s->val;
            }
            s++;
        }
        if((pxcnt_tab == READ_CBUS_REG(ENCI_SYNC_HSO_BEGIN)) &&
           (lncnt_tab == READ_CBUS_REG(ENCI_SYNC_HSO_END))) {
            return 1;
        } else {
            return 0;
        }
    ...
    // check P mode
    while(s->reg != MREG_END_MARKER) {
        if(s->reg == ENCP_VIDEO_MAX_PXCNT) {
            pxcnt_tab = s->val;
        }
        if(s->reg == ENCP_VIDEO_MAX_LNCNT) {
            lncnt_tab = s->val;
        }
        s++;
    }

    if((pxcnt_tab == READ_CBUS_REG(ENCP_VIDEO_MAX_PXCNT)) &&
       (lncnt_tab == READ_CBUS_REG(ENCP_VIDEO_MAX_LNCNT))) {
        return 1;
    } else {
        return 0;
    }
