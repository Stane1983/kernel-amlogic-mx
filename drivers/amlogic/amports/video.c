...
static inline bool vpts_expire(vframe_t *cur_vf, vframe_t *next_vf)
{
    ...
    /* check video PTS discontinuity */
        ...
        if ((int)(systime - pts) >= 0){
            if(next_vf->pts_valid || next_vf->pts != 0){ // to avoid the discontinue mistake because of lookup failed 
...
static ssize_t clear_videobuf_store(struct class *cla, struct class_attribute *attr, const char *buf,
		        size_t count)
{
    unsigned long flags;
    spin_lock_irqsave(&lock, flags);
...
static struct class_attribute amvideo_class_attrs[] = {
	  __ATTR(clear_video_buf,
	  S_IRUGO | S_IWUSR,
	  NULL,
	  clear_videobuf_store),
    __ATTR(axis,
    S_IRUGO | S_IWUSR,
    video_axis_show,
    video_axis_store),
