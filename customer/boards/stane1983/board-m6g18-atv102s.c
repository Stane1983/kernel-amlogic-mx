...
static void vcck_PWM(int power_on) {
    printk("***vcck: vcck_PWM");
...
//        aml_set_reg32_bits(AOBUS_REG_ADDR(0x24), 1, 31, 1);
    ...
    if (power_on) {     
         ...
         gpio_out(PAD_GPIOE_11, 1);//wifi power
         gpio_out(PAD_GPIOAO_2, 0);//led
         gpio_out(PAD_GPIOAO_3, 0);//5v
         vcck_PWM(0);
     }
}
...
MACHINE_START(MESON6_G02, "Amlogic based Visson ATV-102s device")
